   ...
   /* var image = File(picked!.path);*/
    ...
    if (locationData == null) {
      emit(const HomeGetLocationFailed(
          errorMessage: "could not get the location"));
      return;
    }
...
/*void getLocation(GetLocation event, Emitter<HomeState> emit) async {
    LocationData? locationData = await getCurrentLocation();
    if (locationData == null) {
      emit(const HomeGetLocationFailed(
          errorMessage: "could not get the location"));
    ...
    emit(HomeLocationCompleted(
      lattude: locationData.latitude.toString(),
